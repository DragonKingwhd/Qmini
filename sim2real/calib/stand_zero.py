...
吸附加了集体表决门:整圈假设成立时 10 个关节的残差=摆姿态误差,必然集体小;
若是任意复位,残差在 ±π 均匀散布,必然集体大。≥8 个关节残差 ≤ 容限才启用
吸附,否则全部回退 naive(纯姿态反解)并明说。

【2026-06-13 实测结论】断电重上电后残差 4/10 在 ±π 散布 → 此固件【不按】
整圈跳,跨断电吸附永远不会通过,"⚠️不信任吸附→naive"是正常输出不是错误;
naive 就是日常路径(该次 naive 零位经 goto_default 全✅验证)。表决门仍保留:
同一供电会话内重跑时残差≈0 会通过表决,零位原样保留不漂移。
